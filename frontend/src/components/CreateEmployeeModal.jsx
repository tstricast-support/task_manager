...
export default function CreateEmployeeModal({ onClose, onCreate }) {
  ...
  const [department, setDepartment] = useState("I_LAB");   // new
  ...
  return (
    <div className="fixed inset-0 z-30 flex items-center justify-center bg-ink-950/40 px-4" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-xl border border-ink-100 bg-white p-6 shadow-xl">
        <div className="mb-4 flex items-start justify-between">
          <div>
            <h2 className="font-display text-lg font-semibold">Create account</h2>
            <p className="mt-0.5 text-sm text-ink-500">New employees can log in with this ID immediately.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            <label className="mb-1 block text-xs font-medium uppercase tracking-wide text-ink-500">Employee ID</label>
            <input type="text" placeholder="EMP103" value={employeeId} onChange={(e) => setEmployeeId(e.target.value)}
          ...
          <div>
            <label className="mb-1 block text-xs font-medium uppercase tracking-wide text-ink-500">Temporary password</label>
            <input type="text" placeholder="At least 8 characters" value={password} onChange={(e) => setPassword(e.target.value)}
              className="w-full rounded-md border border-ink-200 px-3 py-2 text-sm" />
          </div>
          <div>
            ...
            <select value={department} onChange={(e) => setDepartment(e.target.value)}
              className="w-full rounded-md border border-ink-200 px-3 py-2 text-sm">
              {DEPARTMENTS.filter((d) => d.value !== "OTHERS").map((d) => (
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button type="submit" disabled={submitting} className="transition-base rounded-md bg-ember-500 px-4 py-2 text-sm font-semibold text-white hover:bg-ember-600 disabled:cursor-not-allowed disabled:opacity-60">
